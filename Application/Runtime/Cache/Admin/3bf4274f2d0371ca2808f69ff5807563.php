	...
	<div class="main-container" id="main-container">
		...
		<div class="main-content">
			...
			<div class="page-content">
				...
				<div class="row">
					<div class="col-xs-12">
						<div class="form-horizontal form" role="form" name="<?php echo ($com["id"]); ?>">
							<div class="form-group">
								<label class="col-sm-3 control-label no-padding-right" for="form-field-1"> 广告主名称 </label>

								<div class="col-sm-9">
									<input type="text" id="form-field-1" placeholder="广告主名称" class="col-xs-10 col-sm-5" name="com_name" value="<?php echo ($com["com_name"]); ?>">
								</div>
							</div>
							<div class="form-group">
								...
								<div class="col-sm-9">
									<div id="place_select">
										<select class="width-20 " id="form-field-select-3" name="province">
											...
											<?php if($com['province_id'] == $pro['id']){echo 'selected';} ?>
											><?php echo ($pro["province_name"]); ?></option><?php endforeach; endif; ?>
										</select>
										<select class="width-20 " id="form-field-select-1" name="city">
											<option value="">请选择城市</option>	
											<?php if($com['city_id']){ echo "<option value=".$com['city_id']." selected>".getCity($com['city_id'])."</option>"; } ?>	
										</select>
										<select class="width-20 " id="form-field-select-3" name="district">
											<option value="">请选择区/县</option>
											<?php if($com['district_id']){ echo "<option value=".$com['district_id']." selected>".getDistrict($com['district_id'])."</option>"; } ?>
										</select>
										<select class="width-20 " id="form-field-select-3" name="street">
											<option value="">请选择街道/村</option>
											<?php if($com['street_id']){ echo "<option value=".$com['street_id']." selected>".getStreet($com['street_id'])."</option>"; } ?>
										</select>
										<select class="width-20 " id="form-field-select-3" name="barbershop">
											<option value="">请选择理发店</option>
											<?php if($com['barbershop_id']){ echo "<option value=".$com['barbershop_id']." selected>".getShopname($com['barbershop_id'])."</option>"; } ?>
										</select>
									</div>
								</div>
							</div>
								...
								<div class="col-sm-9">
									<div>
										<select class="width-80 chosen-select" id="form-field-select-3" data-placeholder="Choose a Country..." style="display: none;" name="user_id">
											<option value="">&nbsp;</option>
											<?php if(is_array($user)): foreach($user as $key=>$u): ?><option value="<?php echo ($u["id"]); ?>"
											<?php if($com['user_id'] == $u['id']){echo 'selected';} ?>
											><?php echo ($u["username"]); ?></option><?php endforeach; endif; ?>
										</select>
									</div>
								</div>
							</div>
						...
						<div class="clearfix form-actions">
							<div class="col-md-offset-3 col-md-9">
								<button class="btn btn-info updateCompany" type="button">
									<i class="icon-ok bigger-110"></i>
									修改
								</button>

								&nbsp; &nbsp; &nbsp;
								<button class="btn deleteCompany" type="deleteCompany">
		...
		<script type="text/javascript">
			...
			function deleteCompany(){
				$(".deleteCompany").click(function(){
					...
					$.ajax({
						"url":app+"/Admin/Company/deleteCompany",
						"type":"post",
						"data":{
							"company_id":id},
						"success":function(res){
							if(res.status){
								return dialog.success("删除成功",app+"/Admin/Index/getCompany");
							}
			...
			function updateCompany(){
				$(".updateCompany").click(function(){
					var id = $(".form").attr("name");
					var com_name = $("[name=com_name]").val();
					...
					$.ajax({
						"url":app+"/Admin/Company/updateCompany",
						"type":"post",
						"data":{
							"company_id":id,
							"com_name":com_name,
